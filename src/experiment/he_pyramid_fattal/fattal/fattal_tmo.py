...
def tmo_fattal02(Y, alfa, beta, noise, newfattal, fftsolver, detail_level, hpf_sigma=0.007, selected_levels=None, he_weight=1.0):
    ...
    utils.print_elapsed(f"     [tmo] 가우시안 피라미드 구성 완료 (총 {n_pyramid_levels}개 층)")

    # selected_levels 처리: None인 경우 모든 층 선택
    if selected_levels is None:
        selected_levels_set = set(range(n_pyramid_levels))
    else:
        selected_levels_set = set(selected_levels)

    # value 행렬 계산: 선택된 층만 계산하고 미선택 층은 None으로 설정
    scaling_factor = [None] * n_pyramid_levels
    with ThreadPoolExecutor(max_workers=min(n_pyramid_levels, 9)) as executor:
        futures = []
        for k in range(n_pyramid_levels):
            if k in selected_levels_set and (k >= detail_level or k == n_pyramid_levels - 1 or not newfattal):
    ...
    # exact_continuous_he 히스토그램 평활화 적용
...
def pfstmo_fattal02(img, opt_alpha, opt_beta, opt_noise, newfattal, fftsolver, detail_level, hpf_sigma=0.007, selected_levels=None, he_weight=1.0):
    ...
    L = tmo_fattal02(
        ...
        hpf_sigma=hpf_sigma, selected_levels=selected_levels, he_weight=he_weight
    )
