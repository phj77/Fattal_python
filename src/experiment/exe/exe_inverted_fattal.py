...
def main():
    utils.start_timer()
    utils.print_elapsed("Fattal 톤 매핑 후 흑백 반전 배치 작업 시작")

    data_dir = project_root / "data" / "data_one"
    output_dir = project_root / "test" / "inverted_Fattal"
    ...
    for dataset_num in sorted(DATASET_PARAMS.keys()):
        ...
        for hdr_path in hdr_files:
            ...
            # 흑백 반전 적용 (Invert)
            inverted_img = cv2.bitwise_not(out_img_bgr)

            # 저장 경로 설정 및 저장
            save_path = dataset_output_dir / f"{img_name}.png"
            cv2.imwrite(str(save_path), inverted_img)
