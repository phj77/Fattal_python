...
# 데이터셋별 파라미터 매핑 (alpha, beta) - 이전과 동일하게 유지
DATASET_PARAMS = {
...
# 가우시안 필터 실험용 시그마 강도 5가지
SIGMA_VALUES = [0.5, 1.0, 2.0, 3.0, 5.0]

def main():
    utils.start_timer()
    utils.print_elapsed("Fattal 톤 매핑 후 가우시안 필터 적용 배치 작업 시작")

    data_dir = project_root / "data" / "data_one"
    output_dir = project_root / "test" / "gaussian_Fattal"
    ...
    for dataset_num in sorted(DATASET_PARAMS.keys()):
        ...
        for hdr_path in hdr_files:
            ...
            # 5가지 가우시안 필터 강도 적용 및 저장
            for sigma in SIGMA_VALUES:
                # ksize=(0, 0)으로 지정하면 OpenCV가 sigmaX 값을 기준으로 커널 크기를 자동 계산합니다.
                # 보통 커널 크기는 대략 6 * sigma + 1 범위로 정해집니다.
                blurred_img = cv2.GaussianBlur(out_img_bgr, (0, 0), sigmaX=sigma)
                
                # 저장 경로 설정 및 저장
                save_path = dataset_output_dir / f"{img_name}_sigma_{sigma}.png"
                cv2.imwrite(str(save_path), blurred_img)
                print(f"   => 저장 완료 (sigma={sigma}): {save_path.name}")
