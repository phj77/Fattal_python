# exe_dithering_scanline.py
# Save scanline plots and data for level-wise scaling factors and final combined attenuation map for dithering experiment.
...
# Import dithering modules and configs
from experiment.dithering.fattal.fattal_tmo import (
...
from experiment.dithering.config.config import INPUT_DIR, OUTPUT_DIR, get_parameter_combinations
...
dataset_configs = {
    ...
    3: {"row": 955,  "highlight": [[1400, 1500]]} # data 3
}
...
def process_single_image(img_path, ds_dir, base_output_dir, param):
    ...
    dither_strength = param.get('dither_strength', 0.0)
    pre_hpf_sigma = param.get('pre_hpf_sigma', 0.010)
    
    # We only visualize ds=0.01 per user request or all combinations.
    noise_val = opt_noise if opt_noise > 0 else opt_alpha * 0.01
    
    param_folder_name = f"scanline_preHPF{pre_hpf_sigma}_dither_a{opt_alpha}_b{opt_beta}_ds{dither_strength}"
    
    dataset_output_dir = os.path.join(base_output_dir, "scanlines", dataset_name)
    save_dir = Path(os.path.join(dataset_output_dir, param_folder_name, file_name))
    os.makedirs(save_dir, exist_ok=True)

    print(f"\n--- [Dithering Scanline] Processing: {file_name} ---")
    print(f"  Params: alpha={opt_alpha}, beta={opt_beta}, ds={dither_strength}")
    ...
    for k in range(nlevels):
        if scaling_factors[k] is not None:
            ...
            title = f'Dither({dither_strength}) SF Map - Level {k}/{nlevels-1}\n(size={pw}x{ph}, row={row_k})'
    ...
    final_title = f'Dither({dither_strength}) Final Attenuation Map\n(size={fw}x{fh}, row={scanline_row})'
    ...
    print(f"    [Final Combined] Saved scanline and map at row {scanline_row}")

    # [NEW] Run full TMO and export intermediate scanlines (1_original_HDR_Y to 7_final_LDR_L)
    print(f"    [TMO Run] Executing full TMO with dithering to generate process scanlines...")
    pfstmo_fattal02(
        img_single,
        opt_alpha, opt_beta, opt_noise,
        new_fattal, fft_solver, detail_level,
    ...
    print(f"    [TMO Run] Completed and saved all process scanlines.")



def main():
    utils.start_timer()
    utils.print_elapsed("Dithering Scanline 작업 시작")

    dataset_dirs = validate_and_get_dataset_dirs(INPUT_DIR)
    param_combinations = get_parameter_combinations()
    
    # Filter for ds=0.01 specifically
    param_combinations = [p for p in param_combinations if p.get('dither_strength') == 0.01]
    
    if not param_combinations:
        print("ds=0.01 설정인 파라미터 조합이 없습니다. 모든 조합을 실행합니다.")
        param_combinations = get_parameter_combinations()
    ...
    print(f"실행할 파라미터 수: {len(param_combinations)}\n")
    ...
    for d_idx, ds_dir in enumerate(dataset_dirs, 1):
        ...
        for hdr_file in hdr_files:
            for p_idx, param in enumerate(param_combinations, 1):
                process_single_image(hdr_file, ds_dir, base_output_dir, param)

    utils.print_elapsed("작업 완료")
