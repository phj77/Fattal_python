...
# sys.path에 src 경로 추가하여 공용 utils 모듈 등을 참조 가능하게 함
current_dir = os.path.dirname(os.path.abspath(__file__))
src_dir = os.path.dirname(os.path.dirname(os.path.dirname(current_dir)))
if src_dir not in sys.path:
    sys.path.insert(0, src_dir)

# 사용자 정의 모듈 (이 실험 폴더 내부의 fattal_tmo 참조)
from experiment.fattal_tmp.fattal.fattal_tmo import pfstmo_fattal02

# 파라미터 및 설정 불러오기 (이 실험 폴더 내부의 config 참조)
from experiment.fattal_tmp.config.config import INPUT_DIR, OUTPUT_DIR, get_parameter_combinations
