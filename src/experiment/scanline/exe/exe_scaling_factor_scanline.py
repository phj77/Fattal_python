...
# Fattal 알고리즘 동작 중 생성되는 레벨별 스케일링 팩터(감쇠 맵, Scaling Factor Map)와 최종 합성 감쇠 맵(Combined Attenuation Map, Phi)을 2D 컬러맵 시각화(스캔라인 라인 표시 포함) 및 1D 스캔라인 프로파일 형태로 추출하여 저장하는 실행 스크립트입니다.
...
# Import Fattal modules and configs
from experiment.scanline.fattal.fattal_tmo import (
...
def process_single_image(img_path, ds_dir, base_output_dir, param):
    ...
    # Define save directory structure
    param_folder_name = f"a{opt_alpha}_b{opt_beta}_dl{detail_level}"
    
    # Check if there are other files or if dataset directory structure is nested
    # We will build parameter subdirectories inside dataset output directory
    ...
    print(f"  Params: alpha={opt_alpha}, beta={opt_beta}, noise={noise_val}, detail_level={detail_level}")
    ...
    H = np.log(100.0 * img_single / max_lum + 1e-4)
    ...
    # 3. Calculate level-wise scaling factors
    scaling_factors = [None] * nlevels
    for k in range(nlevels):
        if k >= detail_level or k == nlevels - 1 or not new_fattal:
            att = calculate_level_attenuation(pyramids[k], k, opt_alpha, opt_beta, noise_val)
            scaling_factors[k] = att

    # 4. Calculate final combined attenuation map
    final_att = calculateFiMatrix(scaling_factors, pyramids, nlevels, new_fattal)

    # 5. Global vmin/vmax calculation if FIX_Y_AXIS is True
    if FIX_Y_AXIS:
    ...
    # 6. Generate level-wise scaling factor scanlines and 2D visualizations
    for k in range(nlevels):
    ...
    # 7. Generate final combined attenuation map scanline and 2D visualization
...
def main():
    utils.start_timer()
    utils.print_elapsed("Fattal Scaling Factor 스캔라인 작업 시작")
    ...
    utils.print_elapsed("Fattal Scaling Factor 스캔라인 작업 완료")
