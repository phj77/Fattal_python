...
# 하위 호환성 별칭 (Backward compatibility aliases)
calculate_level_attenuation = calculate_level_scaling_factor
calculateFiMatrix = calculate_attenuation


def tmo_fattal02(Y, alfa, beta, noise, newfattal, fftsolver, detail_level, scanline_row=None, highlight_ranges=None, save_dir=None, hpf_sigma=0.007, scanline_col=None, pyramid_top_size=2**3):
    ...
    #tmp
    attenuation_map[288:325, 1870:1969] *= 10
    
    utils.print_elapsed("     [tmo] FI 행렬 및 그래디언트 계산 완료")

    if scanline_row is not None:
        utils.save_scanline(attenuation_map, scanline_row, "4_attenuation_map", highlight_ranges=highlight_ranges, save_dir=save_dir)
    if scanline_col is not None:
        utils.save_vertical_scanline(attenuation_map, scanline_col, "4_attenuation_map", highlight_ranges=highlight_ranges, save_dir=save_dir)
    ...
    if scanline_row is not None or scanline_col is not None:
        ...
        if scanline_row is not None:
            utils.save_scanline(G_un, scanline_row, "5_original_log_domain_gradient_magnitude", highlight_ranges=highlight_ranges, save_dir=save_dir)
            utils.save_scanline(Gx_un, scanline_row, "5_original_log_domain_GX", highlight_ranges=highlight_ranges, save_dir=save_dir)
            
        if scanline_col is not None:
            utils.save_vertical_scanline(G_un, scanline_col, "5_original_log_domain_gradient_magnitude", highlight_ranges=highlight_ranges, save_dir=save_dir)
            utils.save_vertical_scanline(Gy_un, scanline_col, "5_original_log_domain_GY", highlight_ranges=highlight_ranges, save_dir=save_dir)
    ...
    if scanline_row is not None or scanline_col is not None:
        ...
        if scanline_row is not None:
            utils.save_scanline(G_att, scanline_row, "6_attenuated_gradient_magnitude", highlight_ranges=highlight_ranges, save_dir=save_dir)
            utils.save_scanline(Gx, scanline_row, "6_attenuated_GX", highlight_ranges=highlight_ranges, save_dir=save_dir, ylim=[-0.043, 0.043])
        if scanline_col is not None:
            utils.save_vertical_scanline(G_att, scanline_col, "6_attenuated_gradient_magnitude", highlight_ranges=highlight_ranges, save_dir=save_dir)
            utils.save_vertical_scanline(Gy, scanline_col, "6_attenuated_GY", highlight_ranges=highlight_ranges, save_dir=save_dir)
    ...
    if scanline_row is not None:
        utils.save_scanline(U, scanline_row, "7_before_exponential_U", highlight_ranges=highlight_ranges, save_dir=save_dir)
    if scanline_col is not None:
        utils.save_vertical_scanline(U, scanline_col, "7_before_exponential_U", highlight_ranges=highlight_ranges, save_dir=save_dir)
    ...
    if scanline_row is not None:
        utils.save_scanline(L, scanline_row, "8_final_LDR_L", highlight_ranges=highlight_ranges, save_dir=save_dir, ylim=[0.0, 0.95])
    if scanline_col is not None:
        utils.save_vertical_scanline(L, scanline_col, "8_final_LDR_L", highlight_ranges=highlight_ranges, save_dir=save_dir)
        
    utils.print_elapsed("     [tmo] 정규화 완료")
    
    return L

def pfstmo_fattal02(img, opt_alpha, opt_beta, opt_noise, newfattal, fftsolver, detail_level, scanline_row=None, highlight_ranges=None, save_dir=None, hpf_sigma=0.007, scanline_col=None, pyramid_top_size=2**3):
    ...
    L = tmo_fattal02(img, opt_alpha, opt_beta, opt_noise, newfattal, fftsolver, detail_level, scanline_row=scanline_row, highlight_ranges=highlight_ranges, save_dir=save_dir, hpf_sigma=hpf_sigma, scanline_col=scanline_col, pyramid_top_size=pyramid_top_size)
