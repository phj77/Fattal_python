...
# 사용자 정의 모듈 (Guided Filter + Fattal TMO)
from experiment.guided_filter_fattal.fattal.fattal_tmo import pfstmo_fattal02_guided

# 파라미터 및 설정 불러오기
from experiment.guided_filter_fattal.config.config import (
...
def main():
    utils.start_timer()
    utils.print_elapsed("Guided Filter + Fattal 크롭 실험 실행 시작")
    ...
    for img_path in hdr_files:
        ...
        for p in param_combinations:
            # Guided Filter + Fattal 톤 매핑 연산
            L_out = pfstmo_fattal02_guided(
                img_single,
                ...
                guided_radius=p.get('guided_radius', 16),
                guided_eps=p.get('guided_eps', 0.01),
                add_base_back=p.get('add_base_back', True)
            )
            ...
            ab_str = "ab1" if p.get('add_base_back', True) else "ab0"
            param_suffix = (
                f"a{p['opt_alpha']}_b{p['opt_beta']}_dl{p['detail_level']}"
                f"_gr{p.get('guided_radius', 16)}_ge{p.get('guided_eps', 0.01)}"
                f"_{ab_str}{crop_suffix}"
            )
