...
OUTPUT_DIR = str(test_path / "guided_filter_fattal"/"tmp_Data3_cropped_Base_added")
...
alpha_range_v2 = np.round(np.arange(0.6, 1, 0.3), 2).tolist()

beta_range_v2 = np.round(np.arange(0.5, 1.00, 0.1), 2).tolist()
# ───────────────────────────────────────────────────────────────────
...
PARAM_GRID = {
    ...
    # ─── Guided Filter 파라미터 ───────────────────────────────────
    'guided_radius': [10,12,14,16],       # Guided Filter 윈도우 반경
    'guided_eps': [0.01,0.001,0.1],        # Guided Filter 정규화 파라미터 (Log domain 기준)
    'add_base_back': [True],     # Fattal 처리 후 base layer 재합성 여부
}
