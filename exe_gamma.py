...
opt_saturation = 1  # 채도 복원 강도
opt_noise = 0.001     # 노이즈 레벨 (0.0 이하 입력 시 자동 설정됨)
newfattal = True     # 신규 Fattal 알고리즘 적용 여부
fftsolver = True      # FFT 솔버 사용 (성능 향상을 위해 True 권장)
detail_level = 0      # 디테일 보존 수준 (0 ~ 3)

# 추가된 감마 보정 파라미터
pre_gamma = 1    # 전처리 감마 값 (1.0일 경우 원본 유지)
post_gamma = 1    # 후처리 감마 값 (일반적인 sRGB 모니터 출력을 위해 2.2 사용)
...
cv2.imwrite('output_image_color2.png', out_img_bgr)